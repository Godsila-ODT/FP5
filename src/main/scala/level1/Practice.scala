...
final class WordCount(val filePath: String):
  ...
  def run(): Unit =
    ...
    // val rawText     = readFile(filePatgith)                    // Effect: read
    val source = Source.fromFile(filePath)

    // val text        = normalize(rawText)                    // Pure Logic
    val rawText =
      try source.mkString
    ...
    // val wordList    = words(text)                           // Pure Logic
    val wordList = text.split("\\s+").filter(_.nonEmpty).toList

    // val totalWords  = wordList.size                         // Pure Logic
    val totalWords = wordList.size

    // val uniqueCount = wordList.distinct.size                // Pure Logic
    val uniqueCount = wordList.distinct.size

    // val counts      = countByWord(wordList)                 // Pure Logic
